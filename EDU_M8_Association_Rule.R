...
# Music/ Movie recommendations/ Web Content placement/ recommedations/ optimization
...
itemFrequency(Groceries[,21])

#     onions 
# 0.03101169 

# Total number of transactions = 9835 X 0.03101169

9835 * 0.03101169
# This means that onions shows up in 305 transactions 

itemFrequency(Groceries[,21:27])

itemFrequencyPlot(Groceries[,21:27])
...
#Conclusion: An item that has high support shows more frequently in the data

# Confidence: measures the conditional probability of the
# presence of an item or set of items results in the presence
# of another set of items
# Confidence of buying {Milk} if {Coffee, Sugar} are purchased

# {Coffee, Sugar} -> {Milk} is a rule

# Get the rules

rules <- apriori(Groceries)
# Default para values for supp = 0.001 and conf = 0.8
# No rules were produced for such a low support and conf interval
                 
# which is same as below
rules <- apriori(Groceries, parameter = list(supp = 0.007, conf = 0.25))

rules

# We also dont want less than 2 items in an transaction
rules <- apriori(Groceries, 
                 parameter = list(supp = 0.007, conf = 0.25, minlen = 2))
...
# Lift: It is 3.9 times likely that root vegetables would be purchased 
# with Herbs, than root vegetables by themselves
...
tr<-read.csv("data/train_v2.csv",header=TRUE)
